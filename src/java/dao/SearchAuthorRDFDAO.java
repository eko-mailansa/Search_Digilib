 ...
 * @author AnsaKhitara
 */
public class SearchAuthorRDFDAO extends Object {

    public ArrayList<AuthorRDF> SearchRDFAuthor( String keyword) {
...
//    public static void main(String args[]) throws IOException {
        ...
        String queryString
                ...
                + "PREFIX author: <http://localhost:2020/author/>\n"
                + "PREFIX library: <http://localhost:2020/library/>\n"
                + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
                + "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n"
                + "SELECT ?author_name\n"
                + "WHERE {?author_name a <http://localhost:2020/vocab/author>\n"
                + "FILTER regex(str(?author_name), '"+keyword+"', 'i' )\n"
                + "}\n"
        ...
        while (rs.hasNext()) {
            ...
            RDFNode author_name = qs.get("author_name");
            AuthorRDF Ar = new AuthorRDF(author_name);
            AuthorList.add(Ar);
        }
